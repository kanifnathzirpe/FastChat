...
function Sideabar() {
    ...
    const getAllThreads = async () => {
        try {
            const response = await fetch("https://fast-chat-m6uv.onrender.com/api/thread");
    ...
    const changeThread = async (newThreadId) => {
        ...
        try {
            const response = await fetch(`https://fast-chat-m6uv.onrender.com/api/thread/${newThreadId}`);
            const res = await response.json();
            setPrevChats(res);
            setNewChat(false);
            setReply(null);
    ...
    const deleteThread = async (threadId) => {
        try {
            const response = await fetch(`https://fast-chat-m6uv.onrender.com/api/thread/${threadId}`, {method: "DELETE"});
            const res = await response.json();
            console.log(res);
